"""Deep Temporal ResNet for operations load forecasting with entity embeddings."""
...
class DeepOperationsNet(nn.Module):
    """
    PyTorch Deep Residual Network for multi-series operations forecasting.
    ...
    def __init__(
        ...
        self.head = nn.Sequential(
            ...
            nn.ReLU(),  # Target operational load is strictly non-negative
        )

    def forward(self, x_num: torch.Tensor, s_idx: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x_num: (B, num_continuous_features)
            s_idx: (B,) series index
        Returns:
            (B, 1) predicted load index
        """
        emb = self.series_embedding(s_idx)  # (B, emb_dim)
