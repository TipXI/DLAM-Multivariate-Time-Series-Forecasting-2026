"""Grand Master Ensemble: PyTorch Deep Operations Net (CUDA) + LightGBM + CatBoost on 100% data."""
...
def build_advanced_features(df: pd.DataFrame) -> pd.DataFrame:
    """Multi-scale rolling temporal features, domain physics ratios, and missingness indicators."""
    d = df.copy()
...
def main() -> None:
    ...
    print("=" * 75)
    print(f"GRAND MASTER ENSEMBLE TRAINING (Device: {device.upper()})")
    print("=" * 75)
    ...
    print("\n1. Extracting Multi-Scale Rolling Features across full continuous series...")
    all_combined = pd.concat([train_raw, val_input_raw], ignore_index=True)
    all_featured = build_advanced_features(all_combined)
    ...
    # 2. PyTorch Deep Neural Network (3 seeds on CUDA)
    ...
    # 3. LightGBM High-Capacity Regressors (2 configs)
    print("\n3. Training LightGBM Multi-Scale Regressors...")
    ...
    # 4. CatBoost Regressor with Categorical Series ID
    print("\n4. Training CatBoost Regressor with Categorical Embeddings...")
    ...
    # 5. Tri-Model Grand Blend (40% DeepNet + 35% LightGBM + 25% CatBoost)
    print("\n5. Computing Tri-Model Grand Master Blend...")
    final_preds = 0.40 * dl_preds + 0.35 * lgb_preds + 0.25 * cb_preds
    ...
    output_path = submissions_dir / "grand_master_v3.csv"
    merged_sub.to_csv(output_path, index=False)

    print("\n" + "=" * 75)
    print(f"[SUCCESS] Generated Top-Tier Submission: {output_path}")
    print(f"Summary stats: Min={merged_sub['prediction'].min():.3f}, Max={merged_sub['prediction'].max():.3f}, Mean={merged_sub['prediction'].mean():.3f}")
    print("=" * 75)
