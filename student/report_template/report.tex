...
As shown in Table~\ref{tab:main_results}, our winning model (\texttt{ensemble\_v2}) achieved a validation WAPE of \textbf{14.33\%} (MAE \textbf{1.5779}), substantially outperforming the required naive baseline ($48.10\%$) and seasonal mean baseline ($34.41\%$).
...
\section{Conclusion}
We presented an end-to-end deep learning forecasting system for multivariate operations load prediction. By pairing future operational estimates with missingness-robust indicators, multi-scale rolling temporal features, and an aligned $\ell_1$ loss, our consensus ensemble achieved a validation WAPE of \textbf{14.33\%}, outperforming all baselines. We paired point forecasts with distribution-free conformal prediction intervals and provided rigorous ablation evidence to support each design choice.
