...
Ensemble v2 (\texttt{ensemble\_v2}) & 1.5779 & \textbf{9.3050} & \textbf{3.0504} & 18.51 & 15.88 & 14.33 \\
Grand Master (\texttt{grand\_master\_v3}) & 1.5792 & 9.3261 & 3.0539 & 18.41 & 15.84 & 14.35 \\
\textbf{Winning Model (\texttt{nuhhh\_v5})} & \textbf{1.5739} & 9.3137 & 3.0518 & \textbf{18.35} & \textbf{15.81} & \textbf{14.30} \\
\bottomrule
\end{tabular}%
}
\end{table}

As shown in Table~\ref{tab:main_results}, our winning model (\texttt{nuhhh\_v5}) achieved a validation WAPE of \textbf{14.30\%} (MAE \textbf{1.5739}), substantially outperforming the required naive baseline ($48.10\%$) and seasonal mean baseline ($34.41\%$).
...
\begin{table}[htbp]
\centering
\caption{Generalization benchmark on real Binance Crypto Hourly Trading Volume (Julien, 2023).}
\label{tab:crypto}
\vspace{2mm}
\resizebox{0.95\linewidth}{!}{%
\begin{tabular}{lrrrrr}
\toprule
\textbf{Model Architecture} & \textbf{WAPE} $\downarrow$ & \textbf{MAE} $\downarrow$ & \textbf{MSE} $\downarrow$ & \textbf{RMSE} $\downarrow$ & \textbf{sMAPE (\%)} $\downarrow$ \\
\midrule
Naive Last-Value Baseline & 0.0534 & 0.7108 & 0.8324 & 0.9124 & 5.44 \\
Seasonal Mean Baseline & 0.0523 & 0.6963 & 0.7268 & 0.8525 & 5.31 \\
\textbf{Our PyTorch DeepNet} & \textbf{0.0312} & \textbf{0.4155} & \textbf{0.2732} & \textbf{0.5227} & \textbf{3.22} \\
\bottomrule
\end{tabular}%
}
\end{table}

\paragraph{Domain Transfer Analysis:} On real-world financial time series from Binance across 12 major cryptocurrency pairs (Julien, 2023), our PyTorch DeepNet achieved a WAPE of \textbf{3.12\%}, outperforming the naive baseline ($5.34\%$) and seasonal mean baseline ($5.23\%$) by a substantial \textbf{$>40\%$ error reduction} ($62.4\%$ reduction in MSE). While trading volume exhibits strong intraday cycles, real-world markets experience non-linear liquidity shocks driven by price volatility and returns. Our architecture successfully captures these non-linear volatility-to-volume dependencies and leverages learned entity embeddings to share cross-asset market liquidity dynamics across heterogeneous tokens.

\section{Conclusion}
We presented an end-to-end deep learning forecasting system for multivariate operations load prediction. By pairing future operational estimates with missingness-robust indicators, multi-scale rolling temporal features, and an aligned $\ell_1$ loss, our consensus ensemble achieved a validation WAPE of \textbf{14.30\%} (MAE \textbf{1.5739}), outperforming all baselines. We paired point forecasts with distribution-free conformal prediction intervals and provided rigorous ablation evidence to support each design choice.
